...
public class projDraft {
	...
	// FIX: 
	// Add trim() to strings
	// trim quotations before calculating maximum (done)
	// add field max lengths to last row of file (done)
	// remove src path and make sure it works on lectura (done)
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		try {
			...
			inFileName = usrInput.trim();
			String[] inFileSplit = inFileName.split("\\.");
			System.out.println(inFileSplit[0]);
			outFileName = inFileSplit[0].toString()+".bin";
		...
		*/
		//inFileName = "src/MeteoriteLandings.csv"; //change to prompt user FIX
		//outFileName = "MeteoriteLandings.bin"; // FIX: attach name to .bin extension
		
		// create output file
		//OutputStream outputStream = new FileOutputStream("testingBinaries.bin");
		...
		try {
			while ((line = br.readLine()) != null) {
				...
				inFileContent.add(line.split(",(?=([^\\\"]*\\\"[^\\\"]*\\\")*[^\\\"]*$)",-1)); // don't split by comma its csv bruh
			}
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
...
/*		for (int i = 0; i<20;i++) {
			System.out.println(inFileContent.get(i)[0] + " " + inFileContent.get(i)[1] + " " + inFileContent.get(i)[2] + 
					 " " + inFileContent.get(i)[3] + " " + inFileContent.get(i)[4] + " " + inFileContent.get(i)[5] + " " + 
					inFileContent.get(i)[6] + " " + inFileContent.get(i)[7] + " " + inFileContent.get(i)[8] + " " + inFileContent.get(i)[9]);
		}
*/		
		
		// exclude first row & normalize asciis &  & strip year FIX: 3rd point, double quotes and commas??
		...
		// fix remaining field max lengths && empty fields to -1 to a a new ArrayList<object[]>
